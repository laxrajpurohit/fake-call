...
public class Torch {
    ...
    public static var currentLevel: Float = 0.0
...
//    public static func smoothlyAdjustTorch(from startLevel: Float, to endLevel: Float, duration: TimeInterval) {
...
//          let frameCount = Int(framesPerSecond * duration)
//          let levelChangePerFrame = (endLevel - startLevel) / Float(frameCount)
//
//          var currentLevel = startLevel
//          var currentFrame = 0
//
//          let timer = Timer.scheduledTimer(withTimeInterval: 1.0 / framesPerSecond, repeats: true) { timer in
//              guard currentFrame < frameCount else {
//                  timer.invalidate()
//                  return
//              }
//
//              setTorchLevel(currentLevel)
//              currentLevel += levelChangePerFrame
//              currentFrame += 1
//          }
//
...
//      }
    ...
    static func smoothlyAdjustTorch(from startLevel: Float, to endLevel: Float, duration: TimeInterval) {
        guard isAvailable() else {
            print("Torch not available.")
            return
        }
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 1.0 / framesPerSecond, repeats: true) { timer in
            guard currentFrame < frameCount else {
                timer.invalidate()
                return
            }
    ...
    private static func setTorchLevel(_ level: Float) {
           guard let device = device else { return }
